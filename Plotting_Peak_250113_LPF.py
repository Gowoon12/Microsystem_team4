...
class RealTimeDataCollector:
    def __init__(self):
        # 시리얼 포트 초기화
        self.ser = serial.Serial('COM18', 921600, timeout=0.1) ## COM3, COM18

        self.data = []  # 실시간 데이터를 저장할 리스트 (Vout)
        self.filtered_data = []  # 필터링된 데이터를 저장할 리스트
        self.time = []  # X축 (시간) 리스트

        self.alpha = 0.05  # 필터의 강도를 나타내는 계수 (값을 조정하여 노이즈 수준 조절)
        ...
        self.csv_file_path = os.path.join(directory, f'real_time_data_LPF_{current_time}.csv')
        ...
        self.ax.set_xlim(0, 300)  # X축 범위 설정
    ...
    def update_data(self):
        ...
        if self.ser.in_waiting > 0:
            try:
                ...
                filtered_Vout = (
                    Vout if not self.filtered_data 
                    else self.alpha * Vout + (1 - self.alpha) * self.filtered_data[-1]
                )
    ...
    def animate(self, i):
        ...
        if len(self.time) > 0:
            ...
            if len(self.time) > 300:
                self.ax.set_xlim(len(self.time) - 300, len(self.time))
