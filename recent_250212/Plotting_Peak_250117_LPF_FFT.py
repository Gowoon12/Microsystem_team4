...
class RealTimeDataCollector:
    def __init__(self):
        ...
        # Butterworth 필터 설정
        self.fs = 120  # 샘플링 주파수 (Hz)  
        self.cutoff = 60 # 차단 주파수 (Hz)
        self.order = 3  # 필터 차수
        ...
        with open(self.csv_file_path, mode='w', newline='') as file:
            ...
            writer.writerow(['Vout', 'Filtered_Vout', 'Peak', 'Valley'])  # CSV 파일의 헤더에 피크와 밸리 추가
        ...
        self.ax.set_ylim(10, 15)   # Y축 범위 설정
    ...
    def update_data(self):
        ...
        if self.ser.in_waiting > 0:
            try:
                ...
                Vout = float(data)*50  # 데이터를 float로 변환
                ...
                # 피크 및 밸리 찾기
                self.peaks, _ = find_peaks(self.filtered_data, height=-1, distance=10, prominence=0.05, width=5)
                self.valleys, _ = find_peaks(-np.array(self.filtered_data), height=-1, distance=10, prominence=0.05, width=5)
                ...
                with open(self.csv_file_path, mode='a', newline='') as file:
                    ...
                    writer.writerow([Vout, self.filtered_data[-1] if self.filtered_data else '', peak_value, valley_value])
    ...
    def animate(self, i):
        ...
        return self.filtered_line, self.text#, self.line
